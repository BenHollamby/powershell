...
function Get-OrganisationDetails {
    $owner = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\' `
    -Name RegisteredOwner).RegisteredOwner
    $ownerstring = (Get-culture).TextInfo.ToTitleCase($owner)

    $org = (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\' `
    -Name RegisteredOrganization).RegisteredOrganization
    $orgstring = (Get-culture).TextInfo.ToTitleCase($org)

    Write-Host -ForegroundColor Green "Owner of this machine is: $ownerstring"

    Write-Host -ForegroundColor Green "Organisation is: $orgstring"
}
